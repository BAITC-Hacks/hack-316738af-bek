...
**Біріктірілген өнімде:** түпкі README бойынша репозиторий түбіріндегі `.env` файлын толтырыңыз. `backend.scripts.start` сол файлды оқиды. Төмендегі `ai_engine/.env` жолы AI CLI-ды жеке тексеруге арналған. Render-де осы мәндер сервис Environment бөліміне бөлек енгізіледі; жергілікті файл автоматты жүктелмейді. `APP_ACCESS_TOKEN` — қазыларға берілетін бөлек кіру коды, API кілті емес.
