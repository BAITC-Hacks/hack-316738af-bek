...
**Мәртебе:** backend және frontend-пен біріктірілді. Нақты OpenAI және толық браузер сценарийі тексерілді. Қайталама модель сынақтарында тұрақсыз жағдайлар бар; соңғы нәтижелер мен шектеулер [ортақ тексеру есебінде](../docs/VERIFICATION.md). NVIDIA осы қабылдауда өшірулі.
...
| `prompts.py` | Нұсқаланған нұсқаулар: `qurylym-1.0.2` |
...
`gpt-4.1` осы жобаның live бақылауларында қолданылды; сапасы мен шектері ортақ тексеру есебінде. Ол Responses және Structured Outputs қолдайды. Басқа модель таңдалса бақылаулар қайта орындалуы тиіс. [OpenAI модель құжаты](https://developers.openai.com/api/docs/models/gpt-4.1).
...
Бұл команда 35 тестті, соның ішінде 15 домендік бақылау сценарийін іске қосады және `evaluation/local_reports/engineering_checks.json` жазады. Модельдің орнына ашық көрсетілген scripted test double қолданылады. **Бұл нәтиже нақты AI дәлдігі болып саналмайды.** Тест double public `analyze` арқылы қолданылмайды.
...
1. Нақты OpenAI интеграциясы тексерілді, бірақ қайталама шағын бақылаулардың өзі әр іске қосуда бірдей нәтиже бермеді. Нақты нәтижелер ортақ тексеру есебінде; барлық жаңа құжаттарға дәлдік кепілдігі жоқ.
